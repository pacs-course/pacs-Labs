...
int main()  {
  int thread_id, n_threads, num_threads;

  std::cout << "Type me the number of threads you want to use\n";
  std::cout << "Remember that the system may provide less than the requested\n>";
  std::cin >> num_threads;
  if(num_threads < 1) {
 ...
 */
#pragma omp parallel private(thread_id) num_threads(num_threads) shared(n_threads, num_threads)
  {
