...
/// Measures the computational cost of a function
/**
 * @param f function to be measured
 * @result computational cost of function call
 */
...
main(int argc, char **argv)
{
  ...
  // computes automatically the coefficients of the polynomial
  std::cout << "Coefficients are computed automatically" << std::endl;
  for (unsigned int i = 0; i <= degree; ++i)
    coeff[i] = 2 * std::sin(2.0 * i);

  // generates points to evaluate the functions
