...
class ButcherArray {
 public:
  ...
  // For memory intensive task you should prefer using dense matrices
  // with contiguous memory, for simplicity we use vector<vector>
   ...
   */
  ButcherArray(const unsigned int &n_stages_, const Table_t &A_,
               const std::vector<double> &b1_, const std::vector<double> &b2_,
               const unsigned int &order_)
      : n_stages(n_stages_), A(A_), b1(b1_), b2(b2_), order(order_) {
...
std::unique_ptr<ButcherArray> make_butcher_array(const std::string &type) {
