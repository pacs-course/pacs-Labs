...
class RKFOptions {
 public:
  ...
  template <ScalarOrVector, Scalar>
  friend class RKF;
...
class RKF {
 public:
  ...
  RKF(const Function &function_, const Options &options_ = Options())
      : m_options(options_),
        m_table(
            std::move(RKFScheme::make_butcher_array(m_options.solver_type))),
 ...
 private:
  ...
  Options m_options;
  const std::unique_ptr<ButcherArray> m_table;
  Function m_function;
};

template <ScalarOrVector VariableType, Scalar TimeType>
RKFResult<VariableType, TimeType> RKF<VariableType, TimeType>::solve() const {
  const auto &[t0, tf, y0, h0, tolerance, n_max_steps, factor_reduction,
               factor_expansion, solver_type] = m_options;
...
template <ScalarOrVector VariableType, Scalar TimeType>
std::pair<VariableType, VariableType> RKF<VariableType, TimeType>::RKFstep(
    TimeType t, const VariableType &y, TimeType h) const {
  const auto &n_stages = m_table->n_stages;
