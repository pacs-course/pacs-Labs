...
struct RKFResult {
  ...
  // another way to define ErrorType is
  // using ErrorType = decltype(norm(VariableType{}));
  // This works only if VariableType has a default constructor
...
class RKF {
 public:
   ...
   */
  RKFResult<VariableType, TimeType> solve(TimeType t0, TimeType tf,
                                          const VariableType& y0, TimeType h0,
                                          TimeType tol,
...
RKF<ButcherType, VariableType, TimeType>::solve(
    TimeType t0, TimeType tf, const VariableType& y0, TimeType h0, TimeType tol,
    unsigned int n_max_steps, TimeType factor_reduction,
