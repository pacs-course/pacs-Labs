...
class ButcherArray {
 public:
   ...
   */
  ButcherArray(const Table_t& A_, const std::array<double, N_STAGES>& b1_,
               const std::array<double, N_STAGES>& b2_,
               const unsigned int& order_)
      : A(A_), b1(b1_), b2(b2_), order(order_) {
    c.fill(0.0);
   ...
   * type ButcherArray.
   */
  static constexpr unsigned int n_stages() { return N_STAGES; }
};
