...
\begin{frame}[fragile]{Finite differences}
...
\begin{itemize}
  ...
  \item Define an aggregate class to handle the input options for the \texttt{RKF} class, %eg using composition,
   and provide the corresponding setter/getter methods and possibly a method to parse options from file.
  \item Implement the factory pattern to the \texttt{ButcherArray} class, so that the actual method can be chosen dynamically exploiting polymorphism.
\end{itemize}
\end{frame}
